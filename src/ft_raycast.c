...
void	ft_traverse_map(t_data *data) //habria que a;adirle un segur ppara cuando me salga del mapa. Podria actualizarse las condicoine sdel while.
{
	int		map_x;
	int		map_y;
...
void	ft_rayete(t_data *data, int i)
{
	...
	data->ray->wall_collision.y = data->ray->origin.y + data->ray->length
		* data->ray->dir.y;
	printf("Ray %d: Angle = %.2f, Hit Wall at (NA, NA), Coordinates (%.2f, %.2f), Distance = %.2f\n",
